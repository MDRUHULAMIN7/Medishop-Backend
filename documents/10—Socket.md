...
On success: socket joins a private room -> user:<userId>
    ↓
If role === "admin" || role === "pharmacist" || role === "sales_staff" || role === "inventory_manager": also joins room -> staff
    ↓
...
| `staff` | all connected admin/pharmacist/sales_staff/inventory_manager sessions | new order alerts, prescription alerts, POS/inventory alerts |
...
|---|---|---|---|
| `order:created` | server → `staff` | `{ orderId, userId, total }` | new checkout completed |
...
| `prescription:submitted` | server → `staff` | `{ prescriptionId, userId }` | new prescription upload |
